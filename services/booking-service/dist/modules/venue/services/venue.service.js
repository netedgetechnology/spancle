"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var VenueService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.VenueService = void 0;
const common_1 = require("@nestjs/common");
const event_emitter_1 = require("@nestjs/event-emitter");
const venue_repository_1 = require("../repositories/venue.repository");
const venue_events_1 = require("../events/venue.events");
let VenueService = VenueService_1 = class VenueService {
    constructor(venueRepository, eventEmitter) {
        this.venueRepository = venueRepository;
        this.eventEmitter = eventEmitter;
        this.logger = new common_1.Logger(VenueService_1.name);
    }
    async create(dto, tenantId) {
        this.logger.log(`Creating venue -- tenant: ${tenantId}`);
        const entity = await this.venueRepository.create({ ...dto, tenantId });
        await this.eventEmitter.emitAsync(venue_events_1.VenueEvents.CREATED, { tenantId, venueId: entity.id });
        return entity;
    }
    async findAll(tenantId) {
        return this.venueRepository.findAllByTenant(tenantId);
    }
    async findOne(id, tenantId) {
        const entity = await this.venueRepository.findByIdAndTenant(id, tenantId);
        if (!entity)
            throw new common_1.NotFoundException('Venue not found');
        return entity;
    }
    async update(id, dto, tenantId) {
        await this.findOne(id, tenantId);
        const updated = await this.venueRepository.update(id, tenantId, dto);
        await this.eventEmitter.emitAsync(venue_events_1.VenueEvents.UPDATED, { tenantId, venueId: id });
        return updated;
    }
    async remove(id, tenantId) {
        await this.findOne(id, tenantId);
        await this.venueRepository.softDelete(id, tenantId);
        await this.eventEmitter.emitAsync(venue_events_1.VenueEvents.DELETED, { tenantId, venueId: id });
    }
};
exports.VenueService = VenueService;
exports.VenueService = VenueService = VenueService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [venue_repository_1.VenueRepository,
        event_emitter_1.EventEmitter2])
], VenueService);
//# sourceMappingURL=venue.service.js.map