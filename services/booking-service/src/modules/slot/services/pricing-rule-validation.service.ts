...
import type { PricingRuleEntity, PricingRuleType, ModifierType } from '../entities/pricing-rule.entity';
...
export type ConflictCode =
  | 'DUPLICATE_BASE_RULE'         // Two base rules with overlapping scope + date range
  | 'DUPLICATE_ABSOLUTE_OVERRIDE' // Two absolute custom rules that would both apply
  | 'PRIORITY_COLLISION'          // Same priority, same ruleType, overlapping applicability
  | 'INVALID_PERCENTAGE_RANGE'    // Percentage modifier outside -100% to +10000%
  | 'INVALID_TIME_WINDOW'         // timeStart >= timeEnd
  | 'INVALID_DATE_RANGE'          // validFrom > validUntil
  | 'SCOPE_FK_MISMATCH'          // scope='branch' but branchId=null (etc.)
  | 'SEMANTIC_MISMATCH'          // e.g. 'peak' rule with 'absolute' modifierType
  | 'MEMBER_RULE_ABSOLUTE';       // member discount cannot be absolute (must be % or fixed)
 ...
 */
@Injectable()
export class PricingRuleValidationService {
  private readonly logger = new Logger(PricingRuleValidationService.name);
   ...
   */
  async validate(
    candidate: Partial<PricingRuleEntity>,
    tenantId:  string,
    excludeId?: string,
  ...
  private validateSemantics(
    candidate: Partial<PricingRuleEntity>,
    add:       (e: ConflictEntry) => void,
  ): void {
    const { ruleType, modifierType } = candidate;
    if (!ruleType || !modifierType) return; // partial update — skip
  ...
  private validateScopeConsistency(
    ...
    const checks: Array<[string, string | null | undefined, boolean]> = [
  ...
  private async detectConflicts(
    ...
    const peers = existingRules.filter((r) => {
      if (excludeId && r.id === excludeId) return false; // skip self on update
      return r.ruleType === ruleType;
    });

    for (const peer of peers) {
      if (!this.scopesCouldOverlap(candidate, peer)) continue;
      if (!this.dateRangesOverlap(candidate, peer)) continue;
      if (!this.timeWindowsOverlap(candidate, peer)) continue;
      if (!this.daysOfWeekOverlap(candidate, peer)) continue;
      ...
      if (peer.priority === candidatePriority && (ruleType as string) !== 'base') {
        add({
          level:              'warning',
  ...
  private scopesCouldOverlap(
    a: Partial<PricingRuleEntity>,
    b: PricingRuleEntity,
  ): boolean {
    // tenant-scoped rules overlap with everything
    if (a.scope === 'tenant' || b.scope === 'tenant') return true;

    // same scope type + same FK
    if (a.scope === b.scope) {
      if (a.scope === 'branch') return a.branchId === b.branchId;
      if (a.scope === 'sport')  return a.sportId  === b.sportId;
      if (a.scope === 'court')  return a.courtId  === b.courtId;
    }

    // branch + court: court rule overrides but could overlap semantically
    if (a.scope === 'branch' && b.scope === 'court' && b.branchId === a.branchId) return true;
    if (a.scope === 'court'  && b.scope === 'branch' && a.branchId === b.branchId) return true;
