...
export function isPlatformContractVersionCompatible(received: string): boolean {
  const [rMajor] = received.split('.');
  const [cMajor] = PLATFORM_CONTRACT_VERSION.split('.');
...
export interface PlatformContractEnvelope<TPayload = unknown>
  extends PlatformContractEnvelopeHeader {
